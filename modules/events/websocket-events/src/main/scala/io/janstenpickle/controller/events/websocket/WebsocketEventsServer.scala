...
object WebsocketEventsServer {
  def receive[F[_]: Concurrent, G[_]: MonadCancelThrow, A: Decoder](
    path: String,
  ...
  def send[F[_]: Temporal, G[_]: MonadCancelThrow, A: Encoder](
    path: String,
  ...
  private def make[F[_]: Concurrent, G[_]: MonadCancelThrow](
    path: String,
    suffix: String,
    ...
    def toWebsocket(request: Request[F]) = {
      val lowerName: F ~> G = new (F ~> G) {
        override def apply[A](fa: F[A]): G[A] = k.run(request.uri.path.toString()).use(provide.provide(fa))
      }
