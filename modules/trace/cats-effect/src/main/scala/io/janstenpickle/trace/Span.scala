...
object Span {
  private def setContextSampled(context: SpanContext): SpanContext =
    context.copy(traceFlags = context.traceFlags.copy(sampled = true))
  ...
  def child[F[_]: Sync: Clock](
    ...
    SpanContext.child[F](parent).flatMap { context =>
      sampler
        .shouldSample(Some(parent), parent.traceId, name, kind)
        .ifM(
          Applicative[F].pure(EmptySpan[F](setContextSampled(context))),
          for {
