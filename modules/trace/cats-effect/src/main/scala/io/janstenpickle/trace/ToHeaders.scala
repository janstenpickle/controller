...
object ToHeaders {
  ...
  def apply[F[_]: ToHeaders]: ToHeaders[F] = implicitly

  // TODO extract span kind in headers
  implicit def http[F[_]]: ToHeaders[F] = new ToHeaders[F] {
    ...
    override def toContext(headers: Map[String, String]): Option[SpanContext] = {
      ...
      def stateKv(kv: String): Option[(TraceState.Key, TraceState.Value)] = kv.split('=').toList match {
        case k :: v :: Nil =>
          for {
      ...
      val parseState: TraceState = (for {
        ...
        split = state.split(',')
        traceState <- if (split.length <= 32) TraceState(split.flatMap(stateKv).toMap) else None
    ...
    override def fromContext(context: SpanContext): Map[String, String] = {
      ...
      val traceState = context.traceState.values.map { case (k, v) => show"$k=$v" }.mkString(",")
