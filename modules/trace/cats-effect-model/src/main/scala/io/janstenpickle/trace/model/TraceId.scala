...
object TraceId {
  ...
  def fromHexString(hex: String): Option[TraceId] = Try(Hex.decodeHex(hex)).toOption.flatMap(apply)

  def apply(array: Array[Byte]): Option[TraceId] = if (array.length == 16) Some(new TraceId(array)) else None

  implicit val show: Show[TraceId] = Show.show(tid => Hex.encodeHexString(tid.value))
}
