...
object SpanId {
  ...
  def fromHexString(hex: String): Option[SpanId] = Try(Hex.decodeHex(hex)).toOption.flatMap(apply)

  def apply(array: Array[Byte]): Option[SpanId] = if (array.length == 8) Some(new SpanId(array)) else None

  implicit val show: Show[SpanId] = Show.show(sid => Hex.encodeHexString(sid.value))
}
