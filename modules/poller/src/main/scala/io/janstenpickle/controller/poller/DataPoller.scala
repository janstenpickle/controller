...
object DataPoller {
  ...
  )(create: (() => F[A], A => F[Unit]) => B)(implicit F: Concurrent[F], monoid: Empty[A]): Resource[F, B] = {
  ...
  def apply[F[_]: Timer, A: Empty, B](getData: Data[A] => F[A], pollInterval: FiniteDuration, errorThreshold: PosInt)(
