...
import fs2.Stream
import fs2.Compiler
import io.janstenpickle.controller.configsource.circe.CirceConfigSource.Diff
...
object Events {
  def fromDiff[F[_], A, K, V](
    ...
    removedEvent: (K, V) => A,
    updatedEvent: (K, V) => A
  )(diff: Diff[K, V])(implicit compiler: Compiler[F, F]): F[Unit] =
    Stream
      .emits[F, A](
  ...
  def fromDiff[F[_], A, K, V](publisher: EventPublisher[F, A], addedEvent: (K, V) => A, removedEvent: (K, V) => A)(
    diff: Diff[K, V]
  )(implicit compiler: Compiler[F, F]): F[Unit] =
